...
export default function EnvironmentAnalytics() {
  const avgTemp = (
    mockHelmets.reduce((sum, h) => sum + h.temperature, 0) / mockHelmets.length
  ).toFixed(1);
  const avgHumidity = (
    mockHelmets.reduce((sum, h) => sum + h.humidity, 0) / mockHelmets.length
  ).toFixed(1);
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-foreground-secondary mt-1">
          Environmental conditions monitoring across mining site
        </p>
      </div>
        ...
        <table className="w-full">
          ...
          <tbody>
            {mockHelmets.map((helmet) => {
              const tempStatus =
                helmet.temperature > 35
                  ? "critical"
                  : helmet.temperature >= 30
                    ? "warning"
                    : "success";
              const humStatus =
                helmet.humidity > 75
                  ...
                  : helmet.humidity >= 60
                    ...
                    : "success";
              const overallStatus =
                tempStatus === "critical" || humStatus === "critical"
                  ? "critical"
                  : tempStatus === "warning" || humStatus === "warning"
                    ? "warning"
                    : "success";
