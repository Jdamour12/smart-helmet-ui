...
export default function GasAnalytics() {
  const avgCO = (
    mockHelmets.reduce((sum, h) => sum + h.co, 0) / mockHelmets.length
  ).toFixed(1);
  const avgCH4 = (
    mockHelmets.reduce((sum, h) => sum + h.ch4, 0) / mockHelmets.length
  ).toFixed(2);
  const safeHelmets = mockHelmets.filter(
    (h) => h.co <= 20 && h.ch4 <= 0.8,
  ).length;
  const warningHelmets = mockHelmets.filter(
    (h) => (h.co > 20 && h.co <= 40) || (h.ch4 > 0.8 && h.ch4 <= 1.5),
  ).length;
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-foreground-secondary mt-1">
          CO and Methane level analysis across all helmets
        </p>
      </div>
