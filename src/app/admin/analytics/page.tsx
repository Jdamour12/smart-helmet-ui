...
interface UsageTrendsResponse {
  days: number;
  workers: { date: string; count: number }[];
  supervisors?: { date: string; count: number }[];
  helmets?: { date: string; count: number }[];
}

interface DeptDist { zone: string; count: number; percentage: number; }
interface PeakHour { hour: number; label: string; alert_count: number; }
...
export default function AnalyticsPage() {
  ...
  const sessions   = (sessionsRaw as { total_active_users?: number } | undefined)?.total_active_users ?? 0;
  const workerList = (workersRaw as Worker[] | undefined) ?? [];
  const supList    = (supsRaw as Supervisor[] | undefined) ?? [];

  const trends = trendsRaw as UsageTrendsResponse | undefined;
  let usageTrends: { name: string; workers: number; supervisors: number }[];

  if (trends?.workers?.length) {
    const workerMap = new Map(trends.workers.map((d) => [d.date, d.count]));
    const dates = [...new Set([
      ...trends.workers.map((d) => d.date),
      ...(trends.helmets ?? []).map((d) => d.date),
    ])].sort();

    usageTrends = dates.map((date) => ({
      name: new Date(date).toLocaleDateString([], { weekday: 'short' }),
      workers: workerMap.get(date) ?? 0,
      supervisors: supList.length,
    }));
  } else {
    const days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
    usageTrends = days.map((name) => ({
      name,
  ...
  if (deptRaw && Array.isArray(deptRaw) && deptRaw.length > 0) {
    deptDist = (deptRaw as DeptDist[]).map((d) => ({ name: d.zone, value: d.count }));
  } else {
  ...
  const peakHours = peakRaw && Array.isArray(peakRaw)
    ? (peakRaw as PeakHour[])
        .filter((d) => d.alert_count > 0)
        .map((d) => ({ hour: d.label, usage: d.alert_count }))
    : [];
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Active Sessions',    value: sessions,                              sub: 'Users online',       color: 'primary', Icon: Activity },
          { label: 'Departments',        value: deptDist.length,                       sub: 'Tracked',            color: 'info',    Icon: Layers },
          { label: 'Peak Hours Tracked', value: peakHours.length > 0 ? peakHours.length : '—', sub: 'Alert hotspots', color: 'warning', Icon: Clock },
      ...
      {peakHours.length > 0 ? (
        <div className="bg-background-secondary border border-border rounded-lg p-6">
          <h3 className="text-lg font-semibold text-foreground mb-4">Peak Alert Hours</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {peakHours.slice(0, 8).map((item) => (
              <div key={item.hour} className="text-center p-4 bg-background rounded-lg border border-border/50">
                ...
                <p className="text-xs text-foreground-tertiary mt-1">alerts</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-background-secondary border border-border rounded-lg p-6">
          ...
          <p className="text-sm text-foreground-tertiary text-center py-4">
            Peak hours data will appear once alerts are recorded in the system
          </p>
        </div>
      )}
    </div>
