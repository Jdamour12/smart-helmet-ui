...
export function HelmetCard({ helmet }: HelmetCardProps) {
  const hasCritical = helmet.status === 'alarm';
  const hasAlerts = hasCritical;
  ...
  const signalStatus = getSignalStatus(helmet.signal_strength);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          <h3 className="font-bold text-lg text-slate-900">{helmet.worker_name}</h3>
          <p className="text-xs text-slate-500">ID: {helmet.worker_id}</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        {/* Helmet Worn */}
        <div className="flex items-center gap-2">
          <span className={`w-3 h-3 rounded-full ${helmet.helmet_wear ? 'bg-green-500' : 'bg-red-500'}`}></span>
          <div>
            <p className="text-xs text-slate-600">Helmet</p>
            <p className={`text-sm font-bold ${helmet.helmet_wear ? 'text-green-600' : 'text-red-600'}`}>
              {helmet.helmet_wear ? 'Worn' : 'Not Worn'}
            </p>
          </div>
        </div>
        ...
        {/* Impact Detection */}
        <div className="flex items-center gap-2">
          <span className={`w-3 h-3 rounded-full ${helmet.impact_detected ? 'bg-red-500' : 'bg-green-500'}`}></span>
          <div>
            <p className="text-xs text-slate-600">Impact</p>
            <p className={`text-sm font-bold ${helmet.impact_detected ? 'text-red-600' : 'text-green-600'}`}>
              {helmet.impact_detected ? 'Detected!' : 'Normal'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 text-xs">
        ...
        <div className="flex items-center gap-2">
          <Battery className={`w-4 h-4 ${helmet.battery < 20 ? 'text-red-600' : helmet.battery < 50 ? 'text-yellow-600' : 'text-green-600'}`} />
          <div>
            <p className="text-slate-600">Battery</p>
            <p className="font-semibold">{helmet.battery}%</p>
          </div>
        </div>
      </div>
      ...
      {hasCritical && (
        <div className="mt-4 pt-4 border-t border-slate-200 space-y-2">
          <div className="text-xs p-2 rounded bg-red-100 text-red-700">
            <p className="font-semibold">Helmet status: alarm</p>
          </div>
        </div>
      )}
    </div>
